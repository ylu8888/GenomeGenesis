...
document.addEventListener("DOMContentLoaded", () => {
  const apiKey =
    "d2916117f08364276db8184cad30209e1d56623f7e1445aa8df2d1fc27f0e759";
  ...
  document.querySelector(".search-form").onsubmit = (event) => {
    ...
    if (userInput === "") {
      //if nothing is in search bar just return
    ...
    fetch(
      `https://cors-anywhere.herokuapp.com/https://serpapi.com/search.json?engine=google_scholar&q=${userInput}&api_key=${apiKey}`
    )
      .then((response) => {
      ...
      .then((data) => {
        ...
        for (let i = 0; i < data.organic_results.length; i++) {
          ...
          artItem.style.fontSize = "16px";

          artItem.innerHTML = `<div>
                ...
                Link: <a style="color: #f89880; padding: 3px;" href="${data.organic_results[i].link}" target="_blank" rel="noopener noreferrer">${data.organic_results[i].link}</a>
