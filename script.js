async function fetchPatientData() {
  try {
    const response = await fetch(
      "https://api.cellmodelpassports.sanger.ac.uk/patients?page[size]=10000"
    );
...
async function fetchSampleData() {
  try {
    const response = await fetch(
      "https://api.cellmodelpassports.sanger.ac.uk/samples?page[size]=10000"
    );
    ...
    val_options = [
      ...new Set(
        pd.data
          .filter(
            (item) =>
              item.attributes[xx] !== "NA" &&
              item.attributes[xx] !== "Unknown" &&
              item.attributes[xx] != null
          )
          .map((item) => item.attributes[xx])
      ),
    ];
...
async function sample_patients() {
  ...
  id_list = pd.data
    .filter(
      (item) => item.attributes[xx] == xval && item.attributes[yy] != null
    )
    .map((item) => item.id);
...
async function sp_generate() {
  ...
  if (sxx === "" || sxval === "") {
    ...
    const sxData = sd.data
      .filter((item) => id_list.includes(item.relationships.patient.data.id))
      .filter((item) => item.attributes[sxx] == sxval)
      .map((item) => item.attributes[sxx]);
